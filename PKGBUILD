...
_pkgverbase=3.33.6133.102
_upstreamdate=20260312
_wikidate=20260401
pkgname='ibus-mozc-ut'
pkgver=${_pkgverbase}.u${_upstreamdate}.w${_wikidate}
pkgrel=1
pkgdesc='The Open Source edition of Google Japanese Input bundled with the UT dictionary and IBus module'
arch=('x86_64')
...
noextract=(
    "jawiki-${_wikidate}-pages-articles-multistream-index.txt.bz2"
...
b2sums=(
    'c0bb232f4d9414f703f85772d24b64fca683b100d040df032a82a234f7d76631e5631dcdafb42bd2f306d08db7ea4c38c2751391ecaebee70498999f652a1fbd'
    ...
    'a99b5ff1d18a05e487350c0cbf7cddbc22009989ade8c3599dfc7e0c0d5212d9b09c2d9a3f1979668fa0609aae0108eb31fec7eedae89305eb5964257f24acd1'
...
build() {
    ...
    "${srcdir}"/bazel-8.4.1-linux-x86_64 build server:mozc_server gui/tool:mozc_tool renderer/qt:mozc_renderer unix/ibus:ibus_mozc \
...
package() {
    ...
    install -Dm644 data/images/icon.svg                 "${pkgdir}"/usr/share/"${pkgname}"/product_icon.svg
    install -Dm644 data/images/full_ascii.svg           "${pkgdir}"/usr/share/"${pkgname}"/alpha_full.svg
    install -Dm644 data/images/half_ascii.svg           "${pkgdir}"/usr/share/"${pkgname}"/alpha_half.svg
    install -Dm644 data/images/direct.svg               "${pkgdir}"/usr/share/"${pkgname}"/direct.svg
    install -Dm644 data/images/hiragana.svg             "${pkgdir}"/usr/share/"${pkgname}"/hiragana.svg
    install -Dm644 data/images/full_katakana.svg        "${pkgdir}"/usr/share/"${pkgname}"/katakana_full.svg
    install -Dm644 data/images/half_katakana.svg        "${pkgdir}"/usr/share/"${pkgname}"/katakana_half.svg
    install -Dm644 "${srcdir}"/dictionary.svg           "${pkgdir}"/usr/share/"${pkgname}"/dictionary.svg
    install -Dm644 "${srcdir}"/properties.svg           "${pkgdir}"/usr/share/"${pkgname}"/properties.svg
    install -Dm644 "${srcdir}"/tool.svg                 "${pkgdir}"/usr/share/"${pkgname}"/tool.svg
}
